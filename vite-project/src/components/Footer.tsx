...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-darkes text-slate-100 fixed bottom-0 w-full p-2 flex h-24 sm:h-32 z-10 justify-center items-center">
      <div className="relative mr-4 aspect-square bg-darkest p-2 group flex justify-center items-center sm:w-32 sm:h-32 w-16 h-16">
        {currentBeatImage && <img src={currentBeatImage} alt="Currently playing" className="rounded-md max-w-32 max-h-32 h-full object-scale-down flex items-center justify-center sm:w-28 sm:h-28 w-14" />}
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer" onClick={handlePlayPause}>
      ...
      <div className="flex w-full items-center justify-between gap-1 sm:gap-4 -z-10">
        ...
        <div className="min-w-24 z-10 sm:min-w-28"></div>
